...
# ✅【补丁：防后台丢失 · 完全不阉割】
# ==========================
if "sw_start_real" not in st.session_state:
    st.session_state.sw_start_real = None
if "sw_paused_total" not in st.session_state:
    st.session_state.sw_paused_total = 0
if "sw_is_running" not in st.session_state:
    st.session_state.sw_is_running = False
if "sw_last_project" not in st.session_state:
    st.session_state.sw_last_project = ""
if "sw_last_sub" not in st.session_state:
    st.session_state.sw_last_sub = ""
if "sw_last_type" not in st.session_state:
    st.session_state.sw_last_type = ""

# 实时计算真实时间（退出/后台/锁屏都不掉）
def get_real_elapsed():
    if st.session_state.sw_is_running and st.session_state.sw_start_real is not None:
        return int(time.time() - st.session_state.sw_start_real + st.session_state.sw_paused_total)
    return st.session_state.sw_paused_total
...
# ==========================
if menu == "⏱️ 时间记录":
    st.subheader("⏱️ 学习计时（防丢版 · 锁屏/后台/退出都不掉）")
    ...
    # 保存当前任务
    st.session_state.sw_last_project = main
    st.session_state.sw_last_sub = sub
    st.session_state.sw_last_type = tp

    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("▶️ 开始（防丢）"):
            if not st.session_state.sw_is_running:
                st.session_state.sw_start_real = time.time()
                st.session_state.sw_is_running = True
    with c2:
        if st.button("⏸️ 暂停 / 继续"):
            if st.session_state.sw_is_running:
                st.session_state.sw_paused_total = get_real_elapsed()
                st.session_state.sw_is_running = False
            else:
                st.session_state.sw_start_real = time.time()
                st.session_state.sw_is_running = True
    with c3:
        if st.button("⏹️ 结束并保存（永久记录）"):
            elapsed_total = get_real_elapsed()
            if elapsed_total > 0:
                dur_min = elapsed_total // 60
                start_str = datetime.datetime.fromtimestamp(
                    st.session_state.sw_start_real if st.session_state.sw_start_real else time.time()
                ).strftime("%H:%M:%S")
                end_str = datetime.datetime.now().strftime("%H:%M:%S")

                new_row = pd.DataFrame([{
                    "日期": str(datetime.date.today()),
                    "主项目": st.session_state.sw_last_project,
                    "子项目": st.session_state.sw_last_sub,
                    "时间类型": st.session_state.sw_last_type,
                    "开始时间": start_str,
                    "结束时间": end_str,
                    "总时长(分钟)": dur_min,
                    "备注": ""
                ...
                st.session_state.sw_paused_total = 0
                st.session_state.sw_is_running = False
                st.success("✅ 计时已永久保存！切后台也不会丢！")

    # ✅ 退出重进自动恢复计时
    current_elapsed = get_real_elapsed()
    placeholder = st.empty()
    with placeholder:
        st.metric("⏱ 当前计时（防丢）", fmt(current_elapsed))
